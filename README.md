...
## The conversation, wire-level

Every line below is a real command or response this client sends and parses:
...
CLIENT                                          SERVER
  ...
  │◀─────────────── 220 Go ahead ──────────────────┤
  │═══════════ TLS HANDSHAKE ══════════════════════│
  ├──── EHLO localhost  (re-issued, post-TLS) ────▶│
  │                                                │
  ├──── AUTH LOGIN ────────────────────────────────▶│
  │◀─────────────── 334 base64("Username:") ───────┤
  ├──── base64(email) ─────────────────────────────▶│
  ...
  ├──── [MIME multipart message + attachment] ────▶│
  ├──── <CRLF>.<CRLF>  (end of DATA) ──────────────▶│
